...
describe('Gateway Authentication Offloading & Token Verification Suite', () => {
  ...
  before(async () => {
          ...
          if (credentials.email === 'admin@example.com' && credentials.password === 'AdminPass123!') {
            ...
            res.writeHead(200, {
              ...
              'Set-Cookie': [
                ...
                'refresh_token=mock_refresh_token_abc; Path=/api/auth/refresh; HttpOnly; SameSite=Strict'
              ]
            });
    ...
    // Downstream service relies on Gateway Offloading (trusts X-User-Role and X-User-Id injected by Gateway)
    ...
    // 4. Gateway Simulator adhering strictly to store_gateway NGINX rules:
    // - Strips untrusted X-User-* headers (anti-spoofing)
    // - Uses auth_request subrequest on mutating product endpoints
    // - Injects verified X-User-Id, X-User-Role, X-User-Email
    ...
    gatewaySimulator = http.createServer(async (req, res) => {
      ...
      if (req.url.startsWith('/api/auth/') || req.url === '/.well-known/jwks.json') {
      ...
      if (req.url.startsWith('/api/products') || req.url.startsWith('/api/v1/products')) {
        ...
        // Perform Gateway Auth Subrequest for mutating methods
        if (isMutating) {
        ...
        // Anti-Spoofing: sanitize incoming headers and inject verified values
        const sanitizedHeaders = { ...req.headers };
        delete sanitizedHeaders['x-user-id'];
        delete sanitizedHeaders['x-user-role'];
        delete sanitizedHeaders['x-user-email'];
  ...
  test('Step 2: Admin logs in via Gateway and receives access token', async () => {
    const res = await fetch(`http://127.0.0.1:${gatewayPort}/api/auth/login`, {
      method: 'POST',
  ...
  test('Step 5: Authenticated Admin creates product; Gateway verifies token and injects verified X-User-Role downstream', async () => {
    // 1. Login
    const loginRes = await fetch(`http://127.0.0.1:${gatewayPort}/api/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'admin@example.com', password: 'AdminPass123!' })
    });
  ...
  test('Step 6: Anti-Spoofing: Client cannot inject fake X-User-Role to gain admin access without valid token', async () => {
    ...
    // Gateway rejects because auth subrequest fails (no token)
    assert.equal(res.status, 401);
  });
});
