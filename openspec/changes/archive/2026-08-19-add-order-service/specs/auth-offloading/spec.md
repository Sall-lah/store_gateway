...
The API Gateway SHALL verify authentication tokens on protected routes (including Product Service and Order Service mutating endpoints) by performing an internal subrequest to the Auth Service verification endpoint before allowing downstream forwarding.
...
The API Gateway SHALL strip all client-supplied `X-User-*` headers and ONLY populate downstream `X-User-*` headers with values verified by the gateway's authentication subrequest across all proxied service endpoints.
